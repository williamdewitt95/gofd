...
void createGenericOctogonBuilding(vector<Polygon3d> &model, vector<Polygon3d> &boundingBox){
	...
	int numFloors = (rand() % (maxFloors - minFloors)) + minFloors;
	{
		...
		texs.push_back(Point(floor(2.0*wallHalfWidth/floorHeight), numFloors, 0));
		texs.push_back(Point(floor(2.0*wallHalfWidth/floorHeight),         0, 0));
		texs.push_back(Point(       0                            ,         0, 0));
	}
	...
	// == Sidewalk ==
	{
		...
		points.push_back(Point(  -wallHalfWidth    , -distFromCenter                          ,   0.15));
		points.push_back(Point(  -sidewalkHalfWidth, -distFromCenter - Building::sidewalkWidth,   0.15));
		points.push_back(Point(   sidewalkHalfWidth, -distFromCenter - Building::sidewalkWidth,   0.15));
		points.push_back(Point(   wallHalfWidth    , -distFromCenter                          ,   0.15));
		points.push_back(Point(  -wallHalfWidth    , -distFromCenter                          ,   0.15));
		texs.push_back(Point(0,0,0));
	...
	double curbDarkness = 0.85; // how much darker the curb should be to add a little depth to things
	{
		...
		points.push_back(Point(  -sidewalkHalfWidth, -distFromCenter - Building::sidewalkWidth, Building::sidewalkThickness));
	...
	for(int x=1; x<8; x++){
		...
		sidewalkCurb.setColor(255*curbDarkness,255/7*x*curbDarkness,255/7*(8-x)*curbDarkness);
