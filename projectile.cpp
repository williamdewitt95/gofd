...
void Projectile::update()
{
	if(this->state == MOVING){
		...
		//every 50 frames, add a trail position to the stack
		if (trailInterval == 50) {
			if (trail.size() == 5) {
				trail.erase(0);
			}
			trail.push_back(new Point(this->center.x, this->center.y, this->center.z));
		}
		this->trailInterval++;
...
void Projectile::generateTrail() {
		trail.push_back(Point(  -5,  5,  20));
		trail.push_back(Point(  -5,  5,   0));
		trail.push_back(Point(  -5, -5,   0));
		trail.push_back(Point(  -5, -5,  20));
		trail.push_back(Point(  -5,  5,  20));
}